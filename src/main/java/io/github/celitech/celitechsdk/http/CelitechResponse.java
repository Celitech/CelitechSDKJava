 ...
 */
@Getter
public class CelitechResponse<T> {
  ...
  private final CelitechResponseMetadata metadata;
   ...
   */
  public CelitechResponse(Response response, byte[] bodyBytes, T data) {
    this.data = data;
    this.raw = new RawHttpResponse(bodyBytes, response);
    this.metadata = new CelitechResponseMetadata(response.code(), response.headers().toMultimap());
  }
